document.addEventListener("DOMContentLoaded", function () {
  ...
  document
    .querySelector("#controls button:nth-child(3)")
    .addEventListener("click", toggleFullscreen);
  document
    .querySelector("#controls button:nth-child(4)")
...
function startWatchdog() {
  ...
  player.watchdogInterval = setInterval(() => {
    ...
    if (timeSinceActivity > 300000) {
      console.warn(
        "[Watchdog] No activity for 5 minutes, attempting recovery...",
      );
      recoverPlayer();
    }
    ...
    if (player.mode === "playlist" && player.state === PlayerState.PLAYING) {
      if (
        player.videoEl &&
        !player.videoEl.paused &&
        player.videoEl.readyState >= 2
      ) {
...
function startMemoryCleanup() {
  ...
  player.memoryCleanupInterval = setInterval(() => {
    ...
    if (player.debugEl && player.debugEl.innerHTML.length > 5000) {
      player.debugEl.innerHTML = player.debugEl.innerHTML
        .split("<br>")
        .slice(0, 5)
        .join("<br>");
    }
...
function handleNetworkFailure() {
  ...
  if (player.networkRetryCount <= player.maxNetworkRetries) {
    const backoffDelay = Math.min(
      1000 * Math.pow(2, player.networkRetryCount - 1),
      30000,
    );
    debug(
      `Network failure, retry ${player.networkRetryCount}/${player.maxNetworkRetries} in ${backoffDelay}ms`,
    );
...
function advanceToNext() {
  ...
  debug(
    `Advancing to index ${player.currentIndex} of ${player.playlist.length}`,
  );
...
function playCurrentItem() {
  ...
  if (player.recoveredState && player.recoveredState.itemId === item.id) {
    player.currentItemStartTime =
      Date.now() - (player.recoveredState.elapsedInSlot || 0);
    isResuming = true;
...
function playImage(item, duration, isResuming = false) {
  ...
  const remainingToSchedule = Math.max(
    1000,
    slotDuration - (Date.now() - player.currentItemStartTime),
  );
...
function playVideo(item, duration, isResuming = false) {
  ...
  const handleVideoEnd = () => {
    ...
    if (remainingSlotTime > 100) {
      debug(
        `Video ended (${videoDuration.toFixed(1)}s), keeping last frame for ${(remainingSlotTime / 1000).toFixed(1)}s to complete ${duration}s slot`,
      );
      player.videoEl.pause();
    } else {
  ...
  player.videoEl.onloadeddata = () => {
    const videoDuration = player.videoEl.duration || 0;
    debug(
      `Video loaded, video duration: ${videoDuration.toFixed(1)}s, slot duration: ${duration}s`,
    );

    if (videoDuration < duration) {
      debug(
        `Video shorter than slot (${videoDuration.toFixed(1)}s < ${duration}s) - will hold last frame`,
      );
    }
  ...
  player.videoEl.ontimeupdate = () => {
    if (
      !videoEnded &&
      player.videoEl.currentTime >= player.videoEl.duration - 0.1
    ) {
      handleVideoEnd();
    }
  };
  ...
  const remainingToSchedule = Math.max(
    1000,
    slotDuration - (Date.now() - startTime),
  );
  scheduleNext(remainingToSchedule);
}
function updateInfoDisplay(item) {
  ...
  currentInfoEl.innerHTML = `<strong>${categoryLabel}</strong> - ${itemName}<br>
  ...
  if (player.playlist.length > 1) {
    ...
    const nextCategoryLabel =
      categoryLabels[nextItem.category] || nextItem.category;
    nextInfoEl.textContent = `Suivant: ${nextCategoryLabel} - ${nextItem.name || "Contenu"}`;
  } else {
...
async function fetchPlaylist() {
  ...
  try {
    ...
    if (newMode === "iptv" && data.iptv) {
      ...
      console.log(
        "[OVERLAY DEBUG] IPTV mode - Received overlays:",
        JSON.stringify(player.overlays, null, 2),
      );
    ...
    console.log(
      "[OVERLAY DEBUG] Received overlays:",
      JSON.stringify(player.overlays, null, 2),
    );
    ...
    if (player.playlist.length !== newPlaylist.length) {
      debug(
        `Playlist updated: ${player.playlist.length} -> ${newPlaylist.length} items`,
      );
    }
    ...
    if (player.state === PlayerState.IDLE && player.playlist.length > 0) {
      ...
      if (
        ...
        state.currentIndex < player.playlist.length
      ) {
...
async function switchToIptvMode() {
  ...
  console.log(
    "[IPTV DEBUG] Current player.iptvChannelName:",
    player.iptvChannelName,
  );
...
function loadMpegTsStream(streamUrl, useHlsConvert = false) {
  ...
  const hlsConvertUrl =
    "/player/tv-stream/" + player.screenCode + "?t=" + Date.now();
    ...
    player.mpegtsPlayer.on(
      ...
      function (errorType, errorDetail, errorInfo) {
        ...
        if (!useHlsConvert && player.iptvRetryCount < player.iptvMaxRetries) {
          ...
          debug(
            "Retrying via HLS conversion (attempt " +
              player.iptvRetryCount +
              ")",
          );
...
function capQualityLevel() {
  ...
  if (currentLevel < 0) {
    currentLevel =
      player.hlsInstance.autoLevelCapping >= 0
        ? player.hlsInstance.autoLevelCapping
        : levels.length - 1;
  }
...
function loadHlsStream(streamUrl, useProxy = false) {
  ...
  if (typeof Hls !== "undefined" && Hls.isSupported()) {
    debug(
      "Using HLS.js for OnlineTV stream (attempt " +
        (player.iptvRetryCount + 1) +
        ", proxy: " +
        useProxy +
        ")",
    );
...
function handleNetworkError(streamUrl, data, currentlyUsingProxy = false) {
  ...
  if (player.iptvRetryCount <= player.iptvMaxRetries) {
    ...
    showIptvError(
      "La chaîne n'est pas accessible. Causes possibles :<br><br>• La chaîne est hors ligne ou protégée<br>• L'URL du stream n'est plus valide<br>• Le serveur bloque les connexions externes<br><br>Essayez une autre chaîne ou vérifiez l'URL M3U.",
    );
  }
}
...
function updateQualityIndicator() {
  ...
  const levels = player.hlsInstance.levels;
  const bandwidth =
    player.currentBandwidth || player.hlsInstance.bandwidthEstimate || 0;
...
function updateIptvStatus(status, message) {
  ...
  infoEl.style.cssText =
    "display: block; position: fixed; top: 20px; left: 20px; backdrop-filter: blur(10px); padding: 12px 20px; border-radius: 8px; color: white; font-family: sans-serif; font-size: 14px; z-index: 60;";
...
function showIptvError(message) {
  ...
  if (!errorEl) {
    ...
    errorEl.style.cssText =
      "position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); background: rgba(0,0,0,0.9); padding: 30px 40px; border-radius: 16px; text-align: center; color: white; font-family: sans-serif; z-index: 100; max-width: 400px;";
...
function renderOverlays() {
  ...
  console.log(
    "[OVERLAY DEBUG] Clearing all " + allContainers.length + " containers",
  );
  ...
  overlays.forEach((overlay) => {
    ...
    if (overlay.type === "ticker") {
      ...
      const duration = Math.max(
        10,
        (overlay.message?.length || 50) / (overlay.scroll_speed / 10),
      );
      container.innerHTML = `
...
async function startPlayer() {
  ...
  player.refreshInterval = setInterval(
    fetchPlaylist,
    player.PLAYLIST_REFRESH_INTERVAL,
  );
    ...
    if ("serviceWorker" in navigator) {
      navigator.serviceWorker.addEventListener("message", (event) => {
        ...
        if (event.data.type === "CACHE_STATUS") {
          debug(
            "Cache: " +
              event.data.status.mediaCount +
              " files (" +
              event.data.status.totalSizeMB +
              " MB)",
          );
        }
